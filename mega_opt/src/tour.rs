...
impl BareTour {
    pub fn new(path: Vec<usize>, nodes: Arc<Vec<(f64, f64)>>, primes: Arc<Vec<bool>>, penalty_config: PenaltyConfig) -> BareTour {
        let (cur_len, cur_real_len) = verify_and_calculate_len(&nodes, &path, &primes, penalty_config);

        BareTour { nodes, path, primes, cur_len, cur_real_len, penalty_config }
    }
    ...
    pub fn make_new(&self, path: Vec<usize>) -> BareTour {
        BareTour::new(path, self.nodes.clone(), self.primes.clone(), self.penalty_config)
    }

    pub fn to_tour(&self) -> Tour {
        Tour::new(self.path.clone(), self.nodes.clone(), self.primes.clone(), self.penalty_config)
    }
}
...
impl Tour {
    pub fn new(path: Vec<usize>, nodes: Arc<Vec<(f64, f64)>>, primes: Arc<Vec<bool>>, penalty_config: PenaltyConfig) -> Tour {
        ...
        Tour { nodes, path, primes, inv, per_nodes_edges, cur_len, prefix_lens, prefix_lens_offset, prefix_lens_offset_rev, cur_real_len, penalty_hash, big_lens_count, penalty_config }
    }

    pub fn get_penalty_config(&self) -> PenaltyConfig {
        self.penalty_config
    }
    ...
    pub fn make_new(&self, path: Vec<usize>) -> Tour {
        Tour::new(path, self.nodes.clone(), self.primes.clone(), self.penalty_config)
    }

    pub fn change_penalty(&self, penalty_config: PenaltyConfig) -> Tour {
        Tour::new(self.path.clone(), self.nodes.clone(), self.primes.clone(), penalty_config)
    }
...
mod tests {
    ...
    fn test_tour_check() {
        ...
        let mut tour = Tour::new(tour_path, Arc::new(nodes), Arc::new(primes), PenaltyConfig { base_penalty: 0.2, ..Default::default() });
    ...
    fn test_dist() {
        ...
        let tour = Tour::new(tour_path, nodes.clone(), primes.clone(), PenaltyConfig { base_penalty: 0.2, ..Default::default() });

        for end in 0..100 {
            for start in 0..end {
                for offset in 0..10 {
                    let slow = calculate_len(&nodes, &tour.get_path()[start..end+1], &primes, offset, PenaltyConfig { base_penalty: 0.2, ..Default::default() });
    ...
    fn test_dist_rev() {
        ...
        for end in 0..100 {
            for start in 0..end {
                for offset in 0..10 {
                    let slow = calculate_len(&nodes, &tour.get_path()[start..end+1].iter().map(|x| *x).rev().collect::<Vec<_>>(), &primes, offset, PenaltyConfig { base_penalty: 0.2, ..Default::default() });
