...
fn do_opt2weak(tour: &mut Tour, candidates: &[Vec<(usize, f64)>], prefix: &str) -> Option<Tour> {
    ...
    for i in 0..1_000_000_000 {
        if let Some((new_tour, _)) = do_opt(&mut cur_tour, candidates, 0.0, 3.0, "heavy-", &mut added_v, &mut removed_v, &mut cand_buf, &HashSet::new(), 0) {
...
fn do_opt2c(tour: &mut Tour, candidates: &[Vec<(usize, f64)>], prefix: &str, base_limit: f64) -> Option<Tour> {
    ...
    for i in 0..tour.get_path().len() {
        if let Some((new_tour, _)) = do_opt(&mut cur_tour, candidates, 0.5, base_limit, "heavy-start-", &mut added_v, &mut removed_v, &mut cand_buf, &HashSet::new(), 4) {
    ...
    for i in 0..1_000_000_000 {
        if let Some((new_tour, _)) = do_opt(&mut cur_tour, candidates, 0.0, base_limit, "heavy-", &mut added_v, &mut removed_v, &mut cand_buf, if found_opts >= 0 {
            &no_tabu
        } else {
            &tabu
        }, 0) {
...
fn do_opt2(tour: &mut Tour, candidates: &[Vec<(usize, f64)>], prefix: &str, base_limit: f64) -> Option<Tour> {
    ...
    let mut cur_tour = tour.change_penalty(PenaltyConfig { base_penalty: tour.get_penalty_config().base_penalty * 3.0, length_slope: 0.0, length_min_slope: 0.0 });
    ...
    for i in 0..tour.get_path().len()*2 {
        if let Some((new_tour, _)) = do_opt(&mut cur_tour, candidates, 0.0, base_limit, "heavy-start-", &mut added_v, &mut removed_v, &mut cand_buf, &HashSet::new(), 4) {
            if new_tour.get_len() < cur_tour.get_len() {
                added_v.iter().for_each(|&x| {
                    tabu.insert(x);
                });
...
fn do_opt(tour: &mut Tour, candidates: &[Vec<(usize, f64)>], temp: f64, base_limit: f64, log_prefix: &str, added: &mut Vec<(usize, usize)>, removed: &mut Vec<(usize, usize)>, cand_buf: &mut Vec<usize>,
...
fn main() {
    ...
    for thread_id in opt.n_threads + opt.n_heavy_threads..opt.n_threads + opt.n_heavy_threads + opt.n_weak_threads {
        ...
        let handle = thread::spawn(move || {
            ...
            let mut our_tour_hash = our_tour.hash();
            loop {
                if let Some(new_tour_base) = do_opt2c(&mut our_tour, &our_candidates, &prefix, base_limit) {
                    //println!("new len {}", new_tour.get_len());
                    {
                        let main_tour = main_tour_mutex.lock().unwrap().clone();
