...
/*pub fn load_pi(n_nodes: usize) -> Vec<f64> {
    ...
    for line in file.lines().skip(1) {
        ...
        if parts.len() == 2 {
            out[parts[0].parse()] = parts[1].parse() / 1000.0;
        }
    }
    out
}*/
...
pub fn dist2(nodes: &[(f64, f64)], a: usize, b: usize) -> f64 {
                    ...
                    for &(c2, _) in &candidates[v2] {
                        ...
                        for &i12 in [i1-1, i1+1].iter() {
                            for &i22 in [i2-1, i2+1].iter() {
                                ...
                                added.push((v1, c1));
                                added_sum += dist2(&tour.nodes, v1, c1);
                                added.push((v2, c2));
                                added_sum += dist2(&tour.nodes,  v2, c2);
                                added.push((v12, v22));
                                added_sum += dist2(&tour.nodes,  v12, v22);
                                removed.push((c1, v12));
                                removed_sum += dist2(&tour.nodes,  c1, v12);
                                removed.push((c2, v22));
                                removed_sum += dist2(&tour.nodes,  c2, v22);
                                removed.push((v1, v2));
                                removed_sum += dist2(&tour.nodes,  v1, v2);
                                ...
                                added_sum -= dist(tour.nodes[v12], tour.nodes[v22]);

                                removed_sum -= dist(tour.nodes[c1], tour.nodes[ v12]);
                                removed_sum -= dist(tour.nodes[c2], tour.nodes[ v22]);
...
fn patch(tour: &mut Tour, candidates: &[Vec<(usize, f64)>], temp: f64, base_limit: f64, log_prefix: &str, added: &mut Vec<(usize, usize)>, removed: &mut Vec<(usize, usize)>, _cand_buf: &mut Vec<usize>, mut all_cycle_parts: Vec<Vec<(usize, usize)>>, mut added_sum: f64, mut removed_sum: f64) -> Option<(Tour, f64)> {
    if added_sum - removed_sum > base_limit {
        return None;
    }
...
pub fn do_opt(tour: &mut Tour, candidates: &[Vec<(usize, f64)>], temp: f64, base_limit: f64, log_prefix: &str, added: &mut Vec<(usize, usize)>, removed: &mut Vec<(usize, usize)>, cand_buf: &mut Vec<usize>,
...
//    let mut added = Vec::new();
    ...
    if rng.gen_range(0, 5) == 0 && min_k <= 3{
        ...
        for i in 0..k {
            let mut next_vertex = 0;
            cand_buf.clear();
            cand_buf.extend(candidates[current_vertex].iter().filter(|&&(_c, d)| d <= removed_sum - added_sum + base_limit).map(|&x| x.0));
            for _ in 0..100 {
